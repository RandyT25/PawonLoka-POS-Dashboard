...
export default function TrialForm({ ingredients, allProducts, onBack, onSubmit, saving, stationColor }) {
  const [trialName, setTrialName] = useState("")
  const [notes, setNotes] = useState("")
  const [items, setItems] = useState([{ ingredient_id: "", qty: "", unit: "" }])

  const canSubmit = trialName && items.some(i => i.ingredient_id && parseNum(i.qty) > 0)
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100dvh", background: "#f5f6fa" }}>
      <datalist id="menu-options">{allProducts.map(p=><option key={p.id} value={p.name}/>)}</datalist>
      ...
      <div style={{ padding: "16px 20px", background: "#fff", display: "flex", alignItems: "center", gap: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.05)", position: "sticky", top: 0, zIndex: 10 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 18, fontWeight: 800, color: "#111" }}>R&D / Trial</div>
          <div style={{ fontSize: 13, color: "#666" }}>Test recipes and log ingredients used</div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px 100px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ background: "#fff", padding: 16, borderRadius: 16, boxShadow: "0 2px 6px rgba(0,0,0,0.03)" }}>
          <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#666", marginBottom: 8, textTransform: "uppercase", letterSpacing: 0.5 }}>Menu / Trial Name *</label>
          <input list="menu-options" type="text" value={trialName} onChange={e=>setTrialName(e.target.value)} placeholder="Choose menu or enter new name..." 
          ...
          <input value={notes} onChange={e=>setNotes(e.target.value)} placeholder="e.g. GoFood photoshoot, tasting..." 
            style={{ width: "100%", padding: "12px 14px", border: "1px solid #E8ECF0", borderRadius: 10, fontSize: 15, background: "#f9f9f9" }} />
        </div>

        <div style={{ background: "#fff", padding: 16, borderRadius: 16, boxShadow: "0 2px 6px rgba(0,0,0,0.03)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: "#111" }}>Ingredients Used</div>
            <button onClick={()=>setItems([...items,{ingredient_id:"",qty:"",unit:""}])} 
              style={{ background: "#f0efff", border: "none", color: "#6366F1", fontWeight: 700, fontSize: 13, padding: "6px 12px", borderRadius: 8, cursor: "pointer" }}>
              + Add Item
            </button>
          </div>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, padding: "16px 20px", background: "linear-gradient(rgba(255,255,255,0), rgba(255,255,255,1) 30%)", zIndex: 20 }}>
        <button onClick={handleSubmit} disabled={saving || !canSubmit} 
          style={{ 
            width: "100%", padding: 18, background: canSubmit ? (stationColor || "#6366F1") : "#D1D5DB", color: "#fff", border: "none", borderRadius: 16, 
            fontSize: 16, fontWeight: 800, cursor: canSubmit ? "pointer" : "not-allowed",
            boxShadow: canSubmit ? `0 4px 12px ${stationColor ? stationColor+"33" : "rgba(99,102,241,0.2)"}` : "none",
            opacity: saving ? 0.7 : 1,
            transition: "all 0.2s",
          }}>
          {saving ? "Submitting..." : "Log Trial Items"}
        </button>
      </div>
    </div>
  )
