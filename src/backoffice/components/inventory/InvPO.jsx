...
async function processPaidPO(po, ingMap) {
  ...
  for (const item of po.po_items||[]) {
    ...
    const costPerBase = qtyBase > 0 ? (parseFloat(item.unit_cost)||0) / (qtyBase / parseFloat(item.qty)) * parseFloat(item.qty) / qtyBase : 0
...
ENDOFFILEcat > src/backoffice/components/inventory/InvPO.jsx << 'ENDOFFILE'
import { useState, useEffect } from "react"
import { supabase } from "../../../lib/supabase"
