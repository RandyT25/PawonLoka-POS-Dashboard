...
export default function TopSlowReport() {
  ...
  const [tab,     setTab]     = useState("food") // food | drinks
  ...
  const foodColor  = "#10B981"
  const drinkColor = "#0EA5E9"
  const color = tab === "food" ? foodColor : drinkColor
        ...
        : (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            ...
            <div className="bo-card">
              <div className="bo-card-title" style={{ color }}>
                🏆 Top 10 {tab==="food"?"Makanan":"Minuman"}
              </div>
              {top10.length === 0
                ? <div style={{ color:"#94A3B8", fontSize:13 }}>Belum ada data</div>
                : top10.map((it,i) => <ItemBar key={it.name} it={it} i={i} color={color}/>)
              }
            </div>

            {/* Slow movers */}
            <div className="bo-card">
              <div className="bo-card-title" style={{ color:"#F59E0B" }}>
                ⚠️ Produk Lambat / Tidak Bergerak
              </div>
              {slow.length === 0
                ...
                : slow.map(it => (
                  <div key={it.name} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 12px", borderRadius:10, background:"#FFFBEB", border:"1px solid #FEF3C7", marginBottom:7 }}>
                    <div>
                      ...
                      <div style={{ fontSize:10, color:"#94A3B8", marginTop:2 }}>{fmt(it.revenue)} omset · {it.cat}</div>
                    </div>
