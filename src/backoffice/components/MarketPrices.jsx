...
export default function MarketPrices() {
  ...
  async function load() {
    ...
    setRows(ingList.map(i => {
      ...
      return {
        ...
        base_unit:       i.unit || "gr",
        buy_unit:        conv?.unit || lastPrice?.unit || i.unit || "gr",
        conv_qty:        conv?.qty || lastPrice?.conv_qty || 1,
  ...
  async function saveRow(row) {
    ...
    // 1. Save to market_prices history
    await supabase.from("market_prices").insert({
      id: "MP-" + Date.now() + "-" + row.ingredient_id.slice(-4),
