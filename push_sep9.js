...
async function run() {
  ...
  trackedIds.forEach(id => {
    ...
    if (ing.name === 'Sop Ayam (sub)') {
      added += 8; // Production of 8
    }
    if (ing.name === 'Sate Kambing (sub)') {
      wasted += 3; // GoFood Tongseng un-rung
    }
  ...
  const payload = {
    ...
    data: {
      date: targetDate,
      items: items,
      notes: 'Auto-pushed from assistant (Sept 9th)',
      shift_id: 'full',
      staff_name: 'Randy',
  ...
  if (res.error) {
    ...
    console.log("Successfully Pushed Sep 9th!")
  }
  
  // Insert manual movements
  const sopAyam = allIngredients.find(i => i.name === 'Sop Ayam (sub)')
  const sateKambing = allIngredients.find(i => i.name === 'Sate Kambing (sub)')
  
  await supabase.from('stock_movements').insert([
    { ingredient_id: sopAyam.id, type: 'Production', qty: 8, date: targetDate, notes: 'Production (from paper)' },
    { ingredient_id: sateKambing.id, type: 'Waste', qty: -3, date: targetDate, notes: 'GoFood Tongseng un-rung' }
  ])
