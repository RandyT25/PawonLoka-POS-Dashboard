...
async function run() {
  ...
  const { data: movements } = await supabase.from('stock_movements').select('ingredient_id, type, qty, date, note').eq('date', targetDate)
  const { data: prevRecon } = await supabase.from('staff_submissions').select('*').eq('type', 'daily_recon').eq('data->>date', '2026-09-10').single()
  ...
  trackedNames.forEach(name => {
    ...
    console.log(`[${name}] Opening: ${openingStock}, Masuk: ${added}, Terjual: ${sold}, Waste/SM: ${wasted}, ProdDed: ${pd}, Adj: ${adjust} => Expected: ${expected}`)
  })
