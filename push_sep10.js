...
async function run() {
  ...
  trackedIds.forEach(id => {
    ...
    if (ing.name === 'Telor') {
      adj -= 4; // Kremesan & Siomay tgl 6 (-2 each)
    }
    ...
    if (ing.name === 'Telor') actualQty = 98
  ...
  const payload = {
    ...
    data: {
      date: targetDate,
      items: items,
      notes: 'Auto-pushed from assistant (Sept 10th)',
      shift_id: 'full',
      staff_name: 'Randy',
  ...
  if (res.error) {
    ...
    console.log("Successfully Pushed Sep 10th!")
  }
  
  // Insert manual movements
  const telor = allIngredients.find(i => i.name === 'Telor')
  await supabase.from('stock_movements').insert([
    { ingredient_id: telor.id, type: 'Adjustment', qty: -4, date: targetDate, note: 'Telor Kremesan & Siomay tgl 6' }
  ])
