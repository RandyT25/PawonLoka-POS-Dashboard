  ...
  if (sept4) {
    sept4.data.items.forEach(i => {
       if (i.name.includes("Telor")) {
          i.adj_qty = -1; // Only 1 was the staff meal!
       }
    ...
    sept4.data.notes = sept4.data.notes.replace(
      ...
      "• Telor (-1 Selisih): Terjual 14 butir. 1 butir di-approve sebagai Staff Meal (masuk kolom Adj). Sisa Teori 118, Fisik 117 (Masih hilang 1 butir yang tidak diketahui)."
    );
    ...
    console.log("Sept 4th Staff Meal corrected to 1!");
  }
}
run();
