...
async function run() {
  ...
  const payload = {
    ...
    data: {
      date: targetDate,
      items: items,
      notes: 'Auto-pushed from assistant (Sept 8th)',
      shift_id: 'full',
      staff_name: 'Randy',
  ...
  if (res.error) {
    ...
    console.log("Successfully Pushed Sep 8th!")
  }
  
  // Insert manual waste movements for Sop Ayam and Telor
  const sopAyam = allIngredients.find(i => i.name === 'Sop Ayam (sub)')
  const telor = allIngredients.find(i => i.name === 'Telor')
  
  await supabase.from('stock_movements').insert([
    { ingredient_id: sopAyam.id, type: 'Waste', qty: -1, date: targetDate, notes: 'Waste' },
    { ingredient_id: telor.id, type: 'Waste', qty: -2, date: targetDate, notes: 'Staff meal' }
  ])
